...
const mapFilterModelToApi = (filterModel) => {
	...
	for (const [field, fm] of Object.entries(filterModel)) {
		...
		if (fm.filterType === 'text') {
			const op = ({
				contains:'contains', notContains:'notContains', equals:'equals', notEqual:'notEquals',
				startsWith:'startsWith', endsWith:'endsWith', blank:'isEmpty', notBlank:'notEmpty'
			})[fm.type] || 'contains';
	...
	for (const [field, list] of byField.entries()) {
		...
		for (const f of list) {
			if (f.op === 'in') {
				...
				const filterVal = t === 'number' && f.value != null && f.value !== '' ? Number(f.value) : f.value;
