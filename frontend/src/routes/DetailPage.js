  ...
  return (
    <Stack gap={2} sx={{mx:10}}>
      ...
      <Paper variant="outlined" sx={{ p: 3 }}>
        <Stack direction={{ xs: "column", sm: "row" }} alignItems={{ xs: "start", sm: "center" }} justifyContent="space-between" gap={1}>
          <Stack>
            <Typography variant="h5" sx={{ fontWeight: 700 }}>{title || `Record #${id}`}</Typography>
            <Typography variant="body2" color="text.secondary">
              {record.BodyStyle ?? "—"} • {record.PowerTrain ?? "—"} • {record.Seats ?? "—"} seater
            </Typography>
          </Stack>
          <Chip color="primary" variant="outlined" sx={{ fontSize: 16, px: 1.5 }} label={fmtEUR(record.PriceEuro)} />
        </Stack>
      </Paper>
      ...
      <Paper variant="outlined" sx={{ p: 2 }}>
        ...
        <Divider sx={{ my: 2 }} />
        <Typography variant="subtitle1" sx={{ fontWeight: 700, mb: 1 }}>Powertrain & Charging</Typography>
        ...
        <Divider sx={{ my: 2 }} />
        <Typography variant="subtitle1" sx={{ fontWeight: 700, mb: 1 }}>Commerce</Typography>
